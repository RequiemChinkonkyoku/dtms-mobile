...
export default function ProfileDetailCard({ profileData, onRefreshProfile }) {
  ...
  const onRefresh = React.useCallback(async () => {
    setRefreshing(true);
    try {
      await Promise.all([loadMemberships(), onRefreshProfile()]);
    } catch (error) {
        ...
        <View style={styles.cardContainer}>
          ...
          <View style={styles.tierSection}>
            ...
            {memberships.map((tier, index) => {
              const isCurrentTier =
                profileData.membershipPoints >= tier.requiredPoints &&
                (index === memberships.length - 1 ||
                  profileData.membershipPoints <
                    memberships[index + 1].requiredPoints);
              ...
              if (index < memberships.length - 1) {
                if (profileData.membershipPoints > tier.requiredPoints) {
                  ...
                  const pointsNeededForNextTier =
                    memberships[index + 1].requiredPoints - tier.requiredPoints;
                  progress = Math.min(
                    100,
              ...
              const backgroundColor = getTierBackgroundColor(
                tier.name,
                isCurrentTier
              );
                  ...
                  <View style={styles.tierCardHeader}>
                    <View style={styles.tierInfo}>
                      ...
                      <View>
                        ...
                        {isCurrentTier && (
                          <Text
                            style={[styles.tierLabel, { color: tierColor }]}
                          >
                            Current Tier
                          </Text>
                        )}
                    ...
                    <Text style={[styles.tierDiscount, { color: tierColor }]}>
